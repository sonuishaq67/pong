...
## Features
- **DQN Algorithm**: Uses a Deep Q-Network with a target network for stability.
...
- **Preprocessing**: Grayscale conversion, resizing (64x64), and frame skipping (4 steps per action).
...
python src/train.py
```
Training progress is saved to the `runs/` directory and model weights are saved in `models/latest.pt`.
...
- `batch_size`: 64
- `epsilon_decay`: 0.995
- `step_repeat`: 4 (Frame skipping)

## Architecture
The Q-Network consists of:
1. **Conv1**: 1 input channel (grayscale), 8 output channels, 4x4 kernel, stride 2.
